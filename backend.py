...
class CameraStreamTrack:
    def __init__(self, camera_index, topic_name):
        ...
        try:
            ...
            logger.info(f"[Camera{camera_index}] ROS话题订阅成功: {topic_name}")
    ...
    def stop(self):
        ...
        logger.info(f"[Camera{self.camera_index}] 摄像头已停止")

def detect_available_cameras():
    """检测系统中可用的ROS摄像头话题"""
    available_cameras = []
    
    logger.info("正在检测可用的ROS摄像头话题...")
    
    # 检查ROS是否可用
    try:
        # 使用更短的超时时间
        rospy.wait_for_service('/rosout', timeout=2.0)
        logger.info("ROS master连接成功")
    except rospy.ROSException as e:
        logger.warning(f"ROS master连接失败: {e}")
        logger.info("继续尝试获取话题，但可能无法正常工作")
    ...
    logger.info(f"检测完成！找到 {len(available_cameras)} 个摄像头话题")
...
class LocalBackend:
    ...
    async def websocket_handler(self, request):
        ...
        logger.info(f"新的WebSocket连接，当前连接数: {len(self.websockets)}")
        ...
        try:
            ...
            logger.info(f"WebSocket连接关闭，当前连接数: {len(self.websockets)}")
    ...
    def init_cameras(self):
        ...
        try:
            ...
            for camera_config in available_cameras:
                ...
                logger.info(f"初始化摄像头 {camera_config['index']}: {camera_config['topic_name']}")
        except Exception as e:
    ...
    async def run(self, host='localhost', port=8080):
        ...
        try:
            ...
            logger.info(f"本地后端服务器启动在 http://{host}:{port}")
